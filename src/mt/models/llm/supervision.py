...
def _read_tabular(path: Path) -> pd.DataFrame:
    """Read a csv/parquet file and normalize its column names to unique strings."""
    ...
    if suffix == ".csv":
        ...
        raise ValueError(
            f"Unsupported data source format for {str(path)!r}. Supported file "
            "extensions are '.csv', '.parquet', '.json', and '.jsonl'."
        )

    columns = tuple(_normalize_column_name(column) for column in frame.columns)
    column_index = pd.Index(columns)
    duplicates = tuple(dict.fromkeys(column_index[column_index.duplicated()].tolist()))
    if duplicates:
        raise ValueError(
