...
def score_window_choices(
    model, tokenizer, segmented, target, window, *, device="cpu"
) -> list[dict[str, Any]]:
    """Score the target segment's choices under a truncated context."""

    return score_window_grid(model, tokenizer, segmented, [(target, window)], device=device)[0]


def score_window_grid(
    model, tokenizer, segmented, cells, *, device="cpu", max_batch_tokens=16384
) -> list[list[dict[str, Any]]]:
    """Score (target, window) cells of one session in packed batches."""

    prompts = [build_window_prompt(segmented, target, window) for target, window in cells]
    scored = score_marked_texts(
        model, tokenizer, prompts, device=device, max_batch_tokens=max_batch_tokens
    )
...
def grid_targets(n_segments, n_points, *, strategy="even") -> list[int]:
    """Return target indices covering the session under a named strategy."""
