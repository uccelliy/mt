...
- Don't own: day-to-day implementation (→ `developer`), tests (→ `tester`), data contract (→ `data-expert`), LLM training and cognitive baselines (→ `llm-expert`)
...
- Routing hints: `developer` for general code and glue; `tester` for test runs and quality gates; `data-expert` for the data contract, transforms, and view construction; `llm-expert` for fine-tuning experiments, Centaur comparisons, cognitive model baselines, and RT modeling
