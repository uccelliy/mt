...
| # | 项目 | 探测命令 | 值 |
|---|---|---|---|
| ~~C9~~ | ~~Python 3.10~~ | **已解决**：改用 2023b 的 **3.11.5**，`requires-python` 放宽到 `<3.12`（`uv lock` 零包版本变动）。不再需要找 3.10 | ✓ |
...
### 5.4c E3 在 16GB 卡上的显存约束（2026-08-01 实测）

冒烟测试里 E3 **20/20 全 OOM**，而同一批 session 的 E0 跑通了。逐项排除后的结论：

**不是注意力回退。** V100 上 FlashAttention 和 cuDNN attention 都不可用
（都要求 sm80+），但 mem-efficient 内核**正常选中**：

```
FLASH      不可用（sm80+）      EFFICIENT  可用   峰值 0.08 GiB  ← 选中
CUDNN      不可用（sm80+）      MATH       可用   峰值 7.06 GiB
实际选中   峰值 0.09 GiB
```

`preflight.py` 现已内置这项检查（40k token / 0.15 GiB），每次作业自动跑。

**不是碎片化。** `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` 同样失败，
且失败时 free 只剩 786 KB——内存是真用光了。

**是容量。** 默认 16384 时需要一个 **6.1 GiB 的连续块**，而 E0 峰值本就是
15798/16384 = **96.4%**，没有余量：

| `--batch-tokens` | 结果 |
|---|---|
| 16384（本地一直用的值） | OOM，每个 session 都失败 |
| 8192 | 通过 |
| 4096 | 通过 |

> ⚠️ **协议纪律：`--batch-tokens` 会改变数值。** 同一个 session 在 8192 和
> 4096 下，window 5 的均值是 0.905458 vs 0.924762，差 **0.019**——批次组成不同
> 导致 fp16 累加顺序不同，与 §5.5 讨论的跨架构噪声是同一种东西。
> **所有要互相比较的运行必须用同一个 `--batch-tokens`。** 这条以前没写，
> 因为本地始终是 16384 没变过。

**两条路，正式作业时再定**：

| | `volta16` + 8192 | `volta32` + 16384 |
|---|---|---|
| 可用节点 | 18 个，排队快 | 6 个，排队慢 |
| 与本地 E3 网格的关系 | 批次不同，逐行有 0.02 量级噪声 | **协议完全一致** |
| 速度 | 略慢 | 不变 |

聚合层面两者都可信（L1 已证明 0.02 的逐行噪声聚合后只剩 6e-4）。但若将来
要与本地那批 E3 结果**直接合并**，`volta32` + 16384 更干净。
`smoke_e0_e3.slurm` 默认取 8192，可用 `MT_E3_BATCH_TOKENS` 覆盖。
...
| SDPA 实际后端 | **mem-efficient**（FLASH/cuDNN 在 sm_70 不可用）；40k token 峰值 0.15 GiB，已内置进 preflight | 2026-08-01 |
