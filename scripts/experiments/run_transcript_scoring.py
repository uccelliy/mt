...
# §2.5, in order. Track P now fills the raw one-token readout and its
# format flag; only response time remains Track-S-only.
...
def main():
    ...
    parser.add_argument(
        "--dataset",
        default=None,
        help="Dataset name recorded in the output (default: the data file's parent directory name)",
    )
    parser.add_argument(
        "--condition",
        default="full",
        help="Condition label recorded in the output; keeps ablations in the same tables (§2.5)",
    )
    parser.add_argument(
        "--choice-readout",
        required=True,
        choices=["greedy-unconstrained-1token"],
        help="Explicit ① protocol: full-vocabulary greedy one-token "
        "readout at each teacher-forced human-response prefix; the "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Sample at most this many sessions per experiment (seeded, reproducible)",
    )
    parser.add_argument("--seed", type=int, default=0, help="Sampling seed")
    parser.add_argument(
        "--shard",
        default=None,
        help="'k/n': process every n-th session starting at k (data parallelism across GPUs)",
    )
    parser.add_argument(
        "--resume", action="store_true", help="Skip sessions already in predictions.csv"
    )
    parser.add_argument(
        "--chunk-size", type=int, default=8, help="Sessions scored between output flushes"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="② width: tokens kept per token position, from the full vocabulary (0 disables ②)",
    )
    parser.add_argument(
        "--max-options",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Skip sessions longer than this many chars (memory guard; ~3.3 chars per token)",
    )
    parser.add_argument(
        "--dtype",
        default="auto",
    ...
    parser.add_argument(
        ...
        help="bitsandbytes quantization (CUDA only; report separately from dense results)",
    )
    parser.add_argument(
        "--notify-email",
        default=None,
    ...
    parser.add_argument(
        "--notify-label", default="mt-analysis", help="Analysis name shown in the progress emails"
    )
    parser.add_argument(
        "--output-dir", required=True, help="Run directory receiving the three §2.5 tables"
    )
    parser.add_argument("--summary", default=None, help="Optional per-experiment metric CSV path")
    ...
    print(
        f"scoring {len(pending)} of {len(rows)} sessions on {device} "
...
def score_chunk(model, tokenizer, chunk, device, batch_tokens, top_k, max_options, failures):
    """Score a chunk, isolating and logging any per-session failures."""

    try:
        return score_session_rows(
            model,
            tokenizer,
            chunk,
            device=device,
...
def report(tables, summary_path, failures):
    ...
    print(
        f"paper-compatible token NLL over all scored responses: "
        f"{frame['nll'].sum() / frame['num_tokens'].sum():.4f}"
    )
    print(
        f"macro choice NLL over {len(summary)} experiments: "
...
def summarize_scores(frame):
    ...
    return totals[
        ["paper_token_nll", "macro_choice_nll", "choices", "choice_tokens", "participants"]
    ]
